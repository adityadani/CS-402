...
void *arrivalThread(void *id)
{
	...
	for (i=0;i<num_packets;i++) {
		...
		if(My402ListEmpty(&queue1)){
			...
			PrintStat(getinstanttime());
			fprintf(stdout,"p%d enters Q1", pkt->pkt_id);
...
void *tokenThread(void *id)
{
	...
	while(1) {
		...
		if(token_bucket<B) {
			...
			tokencount++;
			PrintStat(getinstanttime());
			fprintf(stdout,"token t%d arrives, token bucket now has %d tokens",tokencount,token_bucket);
		}
		else {
			PrintStat(getinstanttime());
			tokencount++;
...
void *serverThread(void *id)
{
	...
	for(i=0;i<pkts_to_arrive;i++) {
		...
		PrintStat(getinstanttime());
		fprintf(stdout,"p%d begins service at S, time in Q2 = %.3fms", pkt->pkt_id,
		...
		fprintf(stdout,"p%d departs from S, service time = %.3fms, time in system = %.3fms", pkt->pkt_id,
